...
test_that("launchApp() lists available apps when app_name is NULL", {
  apps <- launchApp() # No app_name provided
...
test_that("launchApp() throws error when no matching app is found", {
  expect_error(launchApp("non_existent_app"),
...
test_that("launchApp() throws error when multiple apps match", {
  ...
  stub(launchApp, "list.dirs", available_apps)

  expect_error(launchApp("app1"), "Multiple apps match", fixed = TRUE)
})

# Test if the function successfully launches the correct app
test_that("launchApp() launches the correct app when exact match is found", {
  ...
  stub(launchApp, "shiny::runApp", mock_runApp)

  # Call launchApp and check if the correct app is "launched"
  launchApp("specific_app_name")
