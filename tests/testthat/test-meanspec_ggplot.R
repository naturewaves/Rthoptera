...
# Test if window length is auto-calculated when auto.wl = TRUE
test_that("Window length is auto-calculated based on sample rate when auto.wl = TRUE", {
  result <- meanspec_ggplot(coryphoda, scale.type = "dB", auto.wl = TRUE)

  # Check if result is a ggplot object
  expect_true("ggplot" %in% class(result))
})

# Test if window length is manually set when auto.wl = FALSE
test_that("Window length is manually set when auto.wl = FALSE", {
  wl_value <- 512
  result <- meanspec_ggplot(coryphoda, scale.type = "dB", auto.wl = FALSE, wl = wl_value)
...
test_that("Plot is correctly generated with dB scale", {
  result <- meanspec_ggplot(coryphoda, scale.type = "dB")
...
test_that("Plot is correctly generated with linear scale", {
  result <- meanspec_ggplot(coryphoda, scale.type = "Linear")
