...
test_that("spectrum_plotly returns a valid list with Plotly and summary data", {
  result <- spectrum_plotly(coryphoda, freq.res = 50, fmin = 1, fmax = 10)
...
test_that("spectrum_plotly includes lines when show.lines is TRUE", {
  result <- spectrum_plotly(coryphoda, show.lines = TRUE)
...
test_that("spectrum_plotly applies plot title correctly", {
  ...
  result <- spectrum_plotly(coryphoda, plot.title = title, italic.title = FALSE)
...
test_that("spectrum_plotly applies italic title when specified", {
  title <- "Test Spectrum Plot"
  result <- spectrum_plotly(coryphoda, plot.title = title, italic.title = TRUE)
...
test_that("spectrum_plotly works correctly when db.shade is FALSE", {
  result <- spectrum_plotly(coryphoda, db.shade = FALSE)
...
test_that("spectrum_plotly calculates total bandwidth correctly when total.bandwidth is TRUE", {
  result <- spectrum_plotly(coryphoda, total.bandwidth = TRUE)
...
test_that("spectrum_plotly limits indices correctly when limit.indices is TRUE", {
  result_false <- spectrum_plotly(coryphoda, limit.indices = FALSE)
  result_true <- spectrum_plotly(coryphoda, limit.indices = TRUE)

  # Compare summary statistics
  expect_false(identical(result_false$summary$spec.ent, result_true$summary$spec.ent),
               info = "Expected different spectral entropy when limit.indices is TRUE")
})

test_that("spectrum_plotly adds parameter annotations when add.params is TRUE", {
  result <- spectrum_plotly(coryphoda, add.params = TRUE)
  ...
  expect_true(any(grepl("Sampling Rate", sapply(annotations, `[[`, "text"))),
              info = "Parameter annotations are missing when add.params is TRUE")
})

test_that("spectrum_plotly adds summary annotations when add.summary is TRUE", {
  result <- spectrum_plotly(coryphoda, add.summary = TRUE)
  ...
  expect_true(any(grepl("Excursion", sapply(annotations, `[[`, "text"))),
              info = "Summary annotations are missing when add.summary is TRUE")
})

test_that("spectrum_plotly hides axis titles when show.x.title and show.y.title are FALSE", {
  result <- spectrum_plotly(coryphoda, show.x.title = FALSE, show.y.title = FALSE)
  ...
  expect_true(x_title_text == "", info = "X-axis title should be an empty string when show.x.title is FALSE")
  expect_true(y_title_text == "", info = "Y-axis title should be an empty string when show.y.title is FALSE")
})

test_that("spectrum_plotly applies custom colors correctly", {
  custom_color_db <- "red"
  custom_color_linear <- "blue"
  result <- spectrum_plotly(coryphoda, color.db = custom_color_db, color.linear = custom_color_linear)
...
test_that("spectrum_plotly applies custom linewidth correctly", {
  custom_linewidth <- 5
  result <- spectrum_plotly(coryphoda, show.lines = TRUE, linewidth = custom_linewidth)
...
test_that("spectrum_plotly adjusts window length wl when odd", {
  freq_res <- 333
  result <- spectrum_plotly(coryphoda, freq.res = freq_res)
  ...
  result_with_params <- spectrum_plotly(coryphoda, freq.res = freq_res, add.params = TRUE)
...
# test_that("spectrum_plotly handles y.position and x.position parameters (limited support)", {
#   result <- spectrum_plotly(coryphoda, y.position = "right", x.position = "top")
#
#   # Extract plot data
#   plot_data <- plotly::plotly_build(result$plot)
#
...
test_that("spectrum_plotly handles x.breaks parameter", {
  x_breaks <- 15
  result <- spectrum_plotly(coryphoda, x.breaks = x_breaks)

  # Extract plot data
  plot_data <- plotly::plotly_build(result$plot)
  tick_vals <- plot_data$x$layout$xaxis$tickvals

  expect_equal(length(tick_vals), x_breaks, info = "Number of x-axis breaks does not match x.breaks parameter")
})
