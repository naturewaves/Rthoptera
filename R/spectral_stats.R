...
#' @param ampMax Numeric, sets the amplitude scaling (1 for relative amplitude,
#'  0 for dB).
...
spectral_stats <- function(wave,
  ...
  if (ampMax == 0) {
    ...
    spec <- meanspec(wave,
      wl = wlen, dB = if (ampMax == 0) "max0" else NULL,
      plot = FALSE
    )
    spEnt <- sh(spec)
    spFlat <- sfm(spec)
  ...
  p <- p %>%
    layout(
      ...
      yaxis = list(
        title = ifelse(ampMax == 1, "Relative Amplitude", "Amplitude (dB)")
      ),
      annotations = list(
