...
temporal_stats_lq <- function(wave,
  ...
  summary_data <- tibble(
    mean.pci = round(mean(motif_data$pci, na.rm = TRUE), 3),
    sd.pci = round(sd(motif_data$pci, na.rm = TRUE), 3),
    mean.tem.exc = round(mean(train_data$tem.exc, na.rm = TRUE), 3),
    sd.tem.exc = round(sd(train_data$tem.exc, na.rm = TRUE), 3),
    mean.dyn.exc = round(mean(train_data$dyn.exc, na.rm = TRUE), 3),
    ...
    mean.n.trains = round(mean(motif_data$n.trains, na.rm = TRUE), 3),
    sd.n.trains = round(sd(motif_data$n.trains, na.rm = TRUE), 3),
    mean.train.rate = round(mean(motif_data$train.rate, na.rm = TRUE), 3),
    sd.train.rate = round(sd(motif_data$train.rate, na.rm = TRUE), 3),
    mean.duty.cycle = round(mean(motif_data$duty.cycle, na.rm = TRUE), 3),
    sd.duty.cycle = round(sd(motif_data$duty.cycle, na.rm = TRUE), 3),
    mean.entropy = round(mean(motif_data$props.ent, na.rm = TRUE), 3),
