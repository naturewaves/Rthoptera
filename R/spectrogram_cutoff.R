...
#'
#' This function generates a spectrogram from a wave object and applies a cutoff
#' to filter out amplitude values below a specified threshold in dBFS. The
#' resulting spectrogram can be plotted using either ggplot2 or base R plotting functions.
#'
#' @param wave A `Wave` object containing the audio data.
#' @param cutoff A numeric value specifying the dB cutoff threshold for the
#' spectrogram. Values below this threshold will be clipped to `NA`.
#' @param plot A logical value indicating whether to plot the resulting spectrogram.
#' Defaults to `FALSE`.
#' @param plot.title An optional string specifying the title of the plot.
#' Defaults to `NULL`.
#' @param ggplot A logical value indicating whether to use ggplot2 for plotting.
#' If `FALSE`, base R plotting will be used to generate the plot. Defaults to `TRUE`.
#' @param noise.red Logical. If TRUE, the mean of each row in the amplityde matrix
#' is subtracted from each of the elements.
#'
#' @return Returns the spectrogram matrix with the cutoff applied. If `plot = TRUE`,
#' a plot of the spectrogram is displayed.
#' @noRd
#' @keywords internal
#'
