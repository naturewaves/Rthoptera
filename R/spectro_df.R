...
#' @param cutoff Amplitude cutoff in dB below the peak. Values below the cutoff
#' will not be visible in the spectrogram.
...
spectro_df <- function(wave, cutoff = -35, overlap = 95, zeropad = 200) {
  ...
  spect_df_cutoff <- spect_df |>
    mutate(amp_cutoff = ifelse(amp < dyn, dyn, amp))
