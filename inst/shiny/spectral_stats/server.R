server = function(input, output, session) {

  # Store reactive values
  values <- shiny::reactiveValues(speciesName = "", callType = "")

  # Update the title whenever the input changes
  shiny::observe({
    values$speciesName <- input$specimen.id
    values$callType <- input$sound.type
  })

  # Observer to update available wave objects in the environment
  shiny::observe({
    wave_names <- base::ls(envir = .GlobalEnv)
    wave_names <- wave_names[sapply(wave_names, function(x) base::inherits(base::get(x, envir = .GlobalEnv), "Wave"))]
    shiny::updateSelectInput(session, "selectedWave", choices = wave_names)
  })

  shiny::observeEvent(input$ampMax, {
    if (input$ampMax == 0) {
      shiny::updateNumericInput(session, "dbth", value = -20, min = -100, max = -3, step = 1)
    } else {
      shiny::updateNumericInput(session, "dbth", value = 0.1, min = 0.01, max = 0.99, step = 0.01)
    }
  })

  # Reactive expression for re-running analysis
  result <- shiny::eventReactive(input$run, {
    shiny::req(input$selectedWave)
    wave <- base::get(input$selectedWave, envir = .GlobalEnv)
    spectral_stats(
      wave = wave,
      specimen.id = input$specimen.id,
      sound.type = input$sound.type,
      temp = input$temp,
      hpf = input$hpf,
      dbth = base::as.numeric(input$dbth),
      total.range = input$total,
      lines = TRUE,
      ampMax = base::as.numeric(input$ampMax),
      robust = input$robust
    )
  })

  output$plotOutput <- shiny::renderUI({
    shiny::req(result())
    plotly::plotlyOutput("plotlyPlot", width = '1150px')
  })

  output$plotlyPlot <- plotly::renderPlotly({
    shiny::req(result())
    result()$plot %>%
      plotly::layout(
        margin = list(l = 80, r = 0, t = 80, b = 80),
        annotations = list(
          list(
            text = input$sound.type,
            font = list(size = 13, color = 'black'),
            showarrow = FALSE, align = 'right',
            x = 0, y = 1.1, xref = 'x', yref = 'paper'
          )
        )
      )
  })

  output$dataOutput <- DT::renderDT({
    shiny::req(result())
    DT::datatable(result()$data,
                  caption = htmltools::tags$caption(
                    style = "caption-side: top; text-align: center;",
                    class = "caption-top",
                    "Spectral Data"
                  ),
                  options = list(
                    pageLength = 1, lengthChange = FALSE, searching = FALSE, paging = FALSE, info = FALSE,
                    columnDefs = list(list(orderable = FALSE, targets = "_all"))
                  )
    )
  })

  # Download data
  output$downloadData <- shiny::downloadHandler(
    filename = function() {
      base::paste0(base::tolower(values$speciesName), "_", base::tolower(values$callType), "_spectral_stats.csv")
    },
    content = function(file) {
      utils::write.csv(result()$data, file, row.names = FALSE)
    }
  )

  # Save data frame in R environment
  shiny::observeEvent(input$saveDataEnv, {
    shiny::req(result(), input$dataName)
    base::assign(input$dataName, result()$data, envir = .GlobalEnv)
    shiny::showModal(shiny::modalDialog(
      title = "Saved",
      base::paste0("Available as '", input$dataName, "' in the R environment."),
      easyClose = TRUE,
      footer = shiny::modalButton("OK")
    ))
  })

  # Save plot
  output$savePlot <- shiny::downloadHandler(
    filename = function() {
      base::paste0(base::tolower(values$speciesName), "_", base::tolower(values$callType), "_spectral_analysis.html")
    },
    content = function(file) {
      htmlwidgets::saveWidget(result()$plot, file, selfcontained = TRUE)
    }
  )

  # Stop app on session end
  session$onSessionEnded(function() {
    shiny::stopApp()
  })

  # Stop app with "Close App" button
  shiny::observeEvent(input$close, {
    shinyjs::runjs("window.close();")
    shiny::stopApp()
  })
}
