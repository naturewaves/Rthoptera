...
![Fig.1. Mean Power Spectrum of the song of Ctenodecticus bolivari obtained via screenshot from Adobe Audition.](../man/figures/ctenodecticus_bolivari_audition_microsoft.png) 

Below is the same plot type produced by *Rthoptera* (using normalized amplitude instead of dB FS), which took 45 seconds to produce, including the selection of the file, changing scale parameters and custom labels on the Y-axis, modifying the output dimensions, and typing down the species name and call type.

![Fig.2. Mean Power Spectrum of the song of Ctenodecticus bolivari obtained with Rthoptera.](../man/figures/ctenodecticus_spectral_plot.png) 
...
![Fig.3. Oscillogram and spectrogram views of a song of Coryphoda albidicollis obtained from Raven Pro.](../man/figures/coryphoda_raven_70sec.png) 

<br> While the output is nice, it is far from customizable, and the user can't control the output parameters either (e.g., image dimensions).
To our knowledge, all the plot in **Raven** are separate views, not allowing to add a Mean Power Spectrum aligned on the side of the spectrogram.
If we wanted to achieve the same output as in the "multiplot" from *Rthoptera*, we would have to manually paste the spectral plot on the side, requiring a manual adjustment of the dimensions which would likely distort the axes labels.
...
First, we will load the data from the *RthopteraSounds* package:
...
Here we can modify some parameters before computing the plot: ![Fig.4. How to use the "multi_plot" app.](../man/figures/multiplot.gif)

<br>

This is the exported image:

![Fig.5. Exported PNG file from the "multi_plot" app in Rthoptera.](../man/figures/coryphoda_rthoptera_10sec.png) <br>
...
Here are some examples on how to obtain interactive plots:
